...
def test_other_symlink_status(tmp_path: Path) -> None:
    data_root = tmp_path / "datasets"
    ...
    other_root.mkdir()
    project_data.parent.mkdir()
    project_data.symlink_to(other_root, target_is_directory=True)

    status = check_project_data_symlink(project_data, data_root)

    assert status["mode"] == "other_symlink"
    assert status["is_symlink"] is True
    assert status["resolves_to_data_root"] is False
...
def test_regular_directory_status(tmp_path: Path) -> None:
    data_root = tmp_path / "datasets"
    project_data = tmp_path / "project" / "data"
    data_root.mkdir()
    project_data.mkdir(parents=True)
